...
/**
 * @route GET /api/user/lsp/DanhSachLSPCon
 * @description Lấy danh sách loại sản phẩm con
 * @access public
 */
LoaiSanPhamUserRoute.get('/DanhSachLSPCon', async (req, res) => {
    try {
        const pageSize = req.query.pageSize ? parseInt(req.query.pageSize) : 0
        const page = req.query.page ? parseInt(req.query.page) : 0
        const { keyword} = req.query
        ...
        const lspcons = await LoaiSanPhamCon.find({ $and: [keywordCondition], TrangThai: TrangThaiTonTai.ChuaXoa }).limit(pageSize).skip(pageSize * page)
